# patched by David Peng 10-29-21
#!/bin/bash
echo "----------------------------------------------------------"
...
#mrc_files="mrc_files/"
# mrc_files="/home/dp823/scratch60/10057/relion/test_img/"
...
if [ "$3" == "new" ]; then
\rm -r ./tmp/
mkdir -p ./tmp/pre_train_labels
cp -r $P_data/pre_train_labels/* ./tmp/pre_train_labels

mkdir -p ./tmp/pre_val_labels
cp -r $P_data/pre_val_labels/* ./tmp/pre_val_labels

# for i in `cat $P_data/train_files.txt`; do
# # cp $P_data/labels/$i ./tmp/labels; done
# cp $P_data/labels/$i ./tmp/labels; done

# for i in `cat $P_data/val_files.txt`; do
# cp $P_data/labels/$i ./tmp/labels; done
...
mkdir -p ./tmp/png1
mkdir -p ./tmp/png2
$anacondapath PSTrain.py ./tmp/ $mrc_files ./tmp/pre_train_labels/ train
$anacondapath PSTrain.py ./tmp/ $mrc_files_val ./tmp/pre_val_labels/ val

rm -r -f ./tmp/pre_train_labels
rm -r -f ./tmp/pre_val_labels
...
echo "python train2.py --dataset $P_data --frontend $fend --checkpoint_step 10 --brightness 0.5  --model $model --num_epochs 100 --h_flip 1 --v_flip 1 --rotation 8   --batch_size 1 --num_val_images 25 --continue_training 0"
