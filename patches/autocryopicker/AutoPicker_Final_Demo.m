% PATCHED - if we clear all vars we cannot pass in mrc through command line
...
% PATCHED - we read this from stdin
str=mrc;
...
% disp(' ');
    ...
    allAreas = [measurements.Area];  % PATCHED - added semicolon
    allPerimeters = [measurements.Perimeter];  % PATCHED - added semicolon
    % Compute circularities.
    circularities = allPerimeters.^2 ./ (4*pi*allAreas);  % PATCHED - added semicolon
    ...
    All_Area = stats.Area;  % PATCHED - added semicolon
    ...
    % PATCHED - 9 is an ASCII tab char
    disp("AUTOCRYOPICKER_DETECTIONS_START")
    disp(['x' 9 'y' 9 'w' 9 'h' 9 'conf']);
