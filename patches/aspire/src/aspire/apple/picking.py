...
class Picker:
    ...
    def run_svm(self, score):
        ...
        classifier_proba = CalibratedClassifierCV(
    ...
    def extract_particles(self, segmentation, segmentation_proba):
        """
        ...
        segmentation_proba = segmentation_proba[
            self.query_size // 2 - 1 : -self.query_size // 2,
            self.query_size // 2 - 1 : -self.query_size // 2,
        ]  # SJHS
        ...
        center[:, [0, 1]] = center[:, [0, 1]] + (self.query_size // 2 - 1) * np.ones(
            center[:, [0, 1]].shape
        )  # SJHS
        center[:, [0, 1]] = center[:, [0, 1]] + (self.query_size // 2 - 1) * np.ones(
            center[:, [0, 1]].shape
        ...
        if self.output_directory is not None:
            ...
            applepick_path_coord = os.path.join(
                self.output_directory, "{}_applepick.coord".format(name_str)
            )  # SJHS
