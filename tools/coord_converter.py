...
class Converter:
    def __init__(self, input_files, from_format=None, decode=None, output_path=None, to_format=None, boxsize=None,
                 cols=None, single_in_file=False, single_out_file=False, output_suffix='', can_overwrite=False):
        ...
        self.output_dir = None if output_path is None else self.expand_path(output_path, mode='output')
    ...
    def get_input_indices(self):
        ...
        if self.from_format in ['star']:
            ...
            indices = {'x': 0 + idx_shift, 'y': 1 + idx_shift, 'w': 2 + idx_shift, 'h': 3 + idx_shift,
                       'conf': 4 + idx_shift, 'mrc_name': mrc_name_idx}
        elif self.from_format in ['tsv', 'coord']:
            indices = {'x': 0 + idx_shift, 'y': 1 + idx_shift, 'conf': 2 + idx_shift, 'mrc_name': mrc_name_idx}
        else:
        ...
        if not all([self.isint(n) or n == 'auto' for n in self.col_overrides]):
            print("ERROR: Input argument -c requires four parameters, each of which can either be positive/negative "
                  "integers indices or 'auto'.")
            sys.exit(1)

        self.col_overrides = [str(n).lower() for n in self.col_overrides]
        user_indicies = {'x': self.col_overrides[0], 'y': self.col_overrides[1], 'w': self.col_overrides[2],
                         'h': self.col_overrides[3], 'conf': self.col_overrides[4], 'mrc_name': self.col_overrides[5]}
        for key in indices:
    ...
    def detect_input_format(self):  # TODO: this happens ONCE at beginning of run; what if some files are of different from_format?
        ...
        for file in self.input_paths:
            ...
            with open(file, mode='r') as f:
                for line in f:
                    ...
                    elif self.hasnumbers(line) and not line.isspace() and not line.startswith('_'):
                        try:
                            x_index = int(header['_rlnCoordinateX']) - 1 if self.indices['x'] is None else self.indices['x']
                            data['x'].append(float(ln[x_index]))
                            y_index = int(header['_rlnCoordinateY']) - 1 if self.indices['y'] is None else self.indices['y']
                            data['y'].append(float(ln[y_index]))
                        except (KeyError, IndexError) as e:
                            print("WARN: Could not parse STAR header values (%s).\n%s" % (e, header))
                            continue
                        try:
                            fom_index = int(header["_rlnAutopickFigureOfMerit"]) - 1 if self.indices['conf'] is None else self.indices['conf']
                            data['conf'].append(float(ln[fom_index]))
                        except (KeyError, IndexError):
                            pass
                        if self.use_single_file_input:
                            try:
                                name_index = int(header["_rlnMicrographName"]) - 1 if self.indices['mrc_name'] is None else self.indices['mrc_name']
            ...
            with open(file, mode='r') as f:
                for line in f:
                    ...
                    if self.hasnumbers(line) and not line.isspace():
                        if self.use_single_file_input:
                            try:
                                data['mrc_name'].append(str(ln[self.indices['mrc_name']]))
                            except IndexError:
                                print("ERROR: Could not find micrograph name column for single-file input.")
                                sys.exit(1)
                        try:
    ...
    def convert_input(self):
        if self.to_format is None or self.output_dir is None:
            print("ERROR: Please specify an output (to) file format (-t) and an output directory.")
            sys.exit(1)
        ...
        if self.to_format in ['box', 'cbox']:
            if self.manual_boxsize is None:
                if any(['w' not in d or 'h' not in d for d in self.input_data]):
                    print("ERROR: Could not find a box size to use for conversion to '%s'. "
                ...
                for i, d in enumerate(self.input_data):
                    ...
                    self.input_data[i]['w'] = self.input_data[i]['h'] = \
                        self.pad_list_of_lists([[], d['x'], d['y']], pad_with=self.manual_boxsize)[0]
                breakpoint()
        ...
        if self.to_format in ['star']:
            ...
            self.convert_to_box(output_names, include_confidence=self.to_format in ['cbox'])
    ...
    def convert_to_box(self, output_names, include_confidence):
        for i, file in enumerate(self.output_paths):
            try:
                d = self.input_data[i]
                x = d['x']  # we still need to subtract half the width here; do this after even_jagged_arr though
                y = d['y']  # ditto for height
                w = d['w']
                h = d['h']
                rows = zip(*self.even_jagged_arr(x, y, w, h))
                rows = [(r[0] - r[2] / 2, r[1] - r[3] / 2, r[2], r[3]) for r in rows]  # indices follow x, y, w, h above
...
if __name__ == '__main__':
    ...
    parser.add_argument('input', help="Path(s) to input particle coordinates; specify one or more files, not a "
    ...
    parser.add_argument('-f', choices=['star', 'box', 'cbox', 'tsv', 'coord'],
                        help="Format FROM which to convert the input"),
    parser.add_argument('-d', choices=['pickle'],
                        help="Method to use to decode or deserialize input (i.e. if input is serialized rather than "
                             ...
                             "<confidence_score>]"),
    parser.add_argument('-t', choices=['star', 'box', 'cbox', 'tsv', 'coord'],
                        help="Format TO which to convert the input")
    parser.add_argument('-b', type=int, help="If output format is BOX or CBOX, this argument either specifies "
                                             "or overrides the box size to be used (required if input does not "
                                             "include a box size)")
    parser.add_argument('-c', metavar=('X_COL', 'Y_COL', 'W_COL', 'H_COL', 'CONF_COL', 'IMAGE_COL'), nargs=6,
    ...
    a = parser.parse_args()
    converter = Converter(a.input, a.f, a.d, a.output_dir, a.t, a.b, a.c,
                          a.single_in, a.single_out, a.suffix, a.overwrite)
